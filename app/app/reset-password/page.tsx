...
function ResetForm() {
  ...
  useEffect(()=>{
    if(searchParams.get('code')) setStage('reset')
  },[])
  ...
  async function handleRequest(){
    ...
    if(error){
      if(error.message.toLowerCase().includes('rate')){
        setMessage('Too many requests. Please wait 60 seconds before trying again.')
        setCooldown(60)
      } else {
  ...
  async function handleReset(){
    ...
    else{setMessage('Password updated! Redirecting...');setTimeout(()=>router.push('/login'),1500)}
    setLoading(false)
  }
        ...
        <div style={{width:'100%',maxWidth:400}}>
          ...
          {stage==='request'&&(
            <>
              ...
              {message&&(
                <div style={{padding:'10px',borderRadius:8,background:'rgba(255,92,58,0.1)',border:'1px solid rgba(255,92,58,0.3)',color:'#ff7a5c',fontSize:'0.82rem',textAlign:'center',marginBottom:12}}>{message}</div>
              )}
              {cooldown>0&&(
                <div style={{padding:'10px',borderRadius:8,background:'rgba(245,200,66,0.08)',border:'1px solid rgba(245,200,66,0.2)',color:'#f5c842',fontSize:'0.82rem',textAlign:'center',marginBottom:12}}>
                  ⏱ Please wait {cooldown}s before requesting another email
                </div>
              )}
              <button onClick={handleRequest} disabled={loading||!email||cooldown>0}
                style={{width:'100%',padding:15,background:cooldown>0?'rgba(255,255,255,0.1)':'#ff5c3a',border:'none',borderRadius:14,color:'white',fontFamily:'Syne,sans-serif',fontWeight:700,fontSize:'1rem',cursor:cooldown>0?'not-allowed':'pointer',opacity:loading||!email||cooldown>0?0.6:1,boxShadow:cooldown>0?'none':'0 6px 24px rgba(255,92,58,0.4)',marginBottom:16}}>
                {loading?'Sending...':cooldown>0?`Wait ${cooldown}s...`:'Send Reset Link →'}
              </button>
              <p style={{color:'#7eabc5',fontSize:'0.85rem',textAlign:'center'}}>
                Remember it? <a href="/login" style={{color:'#ff5c3a',fontWeight:600,textDecoration:'none'}}>Sign in</a>
              </p>
            </>
          )}
          ...
          {stage==='done'&&(
            <div style={{textAlign:'center'}}>
              ...
              <p style={{fontSize:'0.85rem',color:'#7eabc5',lineHeight:1.6,marginBottom:24}}>
