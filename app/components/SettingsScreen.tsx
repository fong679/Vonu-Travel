...
export default function SettingsScreen({user,theme,toggleTheme,supabase}:{user:any;theme:'dark'|'light';toggleTheme:()=>void;supabase:any}) {
  ...
  const Section=({title,children}:{title:string;children:any})=>(
    <div style={{marginBottom:20}}>
      <div style={{fontSize:'0.68rem',textTransform:'uppercase',letterSpacing:'0.12em',color:sub,marginBottom:10,paddingLeft:4}}>{title}</div>
      <div style={{background:card,border:`1px solid ${border}`,borderRadius:14,overflow:'hidden'}}>{children}</div>
    </div>
  )
  ...
  const Row=({icon,label,right,onClick,danger}:{icon:string;label:string;right?:any;onClick?:()=>void;danger?:boolean})=>(
    <div onClick={onClick} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'14px 16px',borderBottom:`1px solid ${border}`,cursor:onClick?'pointer':'default'}}>
      <div style={{display:'flex',alignItems:'center',gap:12}}>
        ...
        <span style={{fontSize:'0.9rem',color:danger?'#ff5c3a':text,fontFamily:'DM Sans,sans-serif'}}>{label}</span>
      </div>
      ...
      <div style={{padding:'20px 24px'}}>
          ...
          <Section title="Profile">
            <div style={{padding:'14px 16px',borderBottom:`1px solid ${border}`}}>
              ...
              <div style={{display:'flex',gap:8}}>
                <input value={name} onChange={e=>setName(e.target.value)} placeholder="Your name" style={{flex:1,padding:'10px 12px',background:dark?'rgba(255,255,255,0.05)':'rgba(0,0,0,0.04)',border:`1px solid ${border}`,borderRadius:8,color:text,fontSize:'0.9rem',outline:'none',fontFamily:'DM Sans,sans-serif'}}/>
                <button onClick={handleSaveName} style={{padding:'10px 16px',background:savedName?'#00a550':'#ff5c3a',border:'none',borderRadius:8,color:'white',fontFamily:'Syne,sans-serif',fontWeight:700,fontSize:'0.8rem',cursor:'pointer',transition:'background 0.3s'}}>{savedName?'Saved!':'Save'}</button>
              </div>
            </div>
            ...
            {showReset&&(
              <div style={{padding:'14px 16px',borderBottom:`1px solid ${border}`}}>
                <div style={{fontSize:'0.72rem',textTransform:'uppercase',letterSpacing:'0.1em',color:sub,marginBottom:6}}>Send reset link to</div>
                <div style={{display:'flex',gap:8}}>
                  <input value={resetEmail} onChange={e=>setResetEmail(e.target.value)} placeholder={user.email} type="email" style={{flex:1,padding:'10px 12px',background:dark?'rgba(255,255,255,0.05)':'rgba(0,0,0,0.04)',border:`1px solid ${border}`,borderRadius:8,color:text,fontSize:'0.9rem',outline:'none',fontFamily:'DM Sans,sans-serif'}}/>
        ...
        {user?(
          ...
          <div style={{textAlign:'center',padding:'20px 0'}}>
            <a href="/login" style={{display:'inline-block',padding:'13px 32px',background:'#ff5c3a',borderRadius:14,color:'white',fontFamily:'Syne,sans-serif',fontWeight:700,fontSize:'0.95rem',textDecoration:'none',boxShadow:'0 6px 24px rgba(255,92,58,0.4)'}}>Sign In to your account</a>
          </div>
        )}
      </div>
    </div>
...
EOFcat > components/SettingsScreen.tsx << 'EOF'
'use client'
import { useState } from 'react'
export default function SettingsScreen({user,theme,toggleTheme,supabase}:{user:any;theme:'dark'|'light';toggleTheme:()=>void;supabase:any}) {
  const dark=theme==='dark'
