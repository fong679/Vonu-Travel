...
type Booking = {
  ...
  passengers:any; created_at:string
}
...
type Route = {
  ...
  seats_cabin:number; days_of_week:number[]; active:boolean
  created_at:string
}
...
export default function OperatorDashboard({user,profile,supabase}:{user:any;profile:any;supabase:any}) {
  ...
  function resetRouteForm(){
    setRouteForm({origin:'Suva',destination:'Savusavu',ferry_name:'',operator_name:'',departs:'08:00',arrives:'18:00',duration:'',economy_fare:0,cabin_fare:0,seats_economy:0,seats_cabin:0,days_of_week:[],active:true})
  }
  ...
  const filteredBookings=useMemo(()=>{
    let b=[...bookings]
    if(filterStatus!=='all') b=b.filter(x=>filterStatus==='pending'?x.cancellation_requested:x.status===filterStatus)
  ...
  function Badge({status,pending:p}:{status:string;pending?:boolean}){
    ...
    if(p) return <span style={{fontSize:'0.7rem',color:'#f5c842',background:'rgba(245,200,66,0.1)',padding:'2px 8px',borderRadius:8,border:'1px solid rgba(245,200,66,0.25)'}}>⏳ Pending Cancel</span>
  ...
  function BookingCard({b}:{b:Booking}){
    ...
    return (
      <div style={{background:card,border:`1px solid ${b.cancellation_requested?'rgba(245,200,66,0.3)':border}`,borderRadius:12,marginBottom:8,overflow:'hidden'}}>
        <div onClick={()=>setExpandedId(isExpanded?null:b.id)} style={{padding:'12px 16px',cursor:'pointer'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:6}}>
            ...
            <div style={{textAlign:'right',display:'flex',flexDirection:'column',alignItems:'flex-end',gap:4}}>
              <Badge status={b.status} pending={b.cancellation_requested}/>
        ...
        {isExpanded&&(
          <div style={{borderTop:`1px solid ${border}`,padding:'14px 16px'}}>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'10px 12px',marginBottom:14}}>
              {[{l:'REF',v:b.ref},{l:'Phone',v:b.phone},{l:'ID',v:b.passenger_id},{l:'Operator',v:b.ferry_operator},{l:'Class',v:b.selected_class},{l:'Passengers',v:b.passengers?`${b.passengers.adults||1}A ${b.passengers.children||0}C ${b.passengers.infants||0}I`:'1 Adult'}].map(i=>(
                <div key={i.l}>
            ...
            <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
              ...
              {b.cancellation_requested&&(
                <>
                  <button onClick={()=>rejectCancellation(b.id)} style={{flex:1,padding:'9px',borderRadius:8,border:'1px solid rgba(0,165,80,0.3)',background:'rgba(0,165,80,0.08)',color:'#4dd882',fontFamily:'Syne,sans-serif',fontWeight:600,fontSize:'0.8rem',cursor:'pointer'}}>↩ Reject Cancel</button>
                  <button onClick={()=>confirmCancellation(b.id)} style={{flex:1,padding:'9px',borderRadius:8,border:'1px solid rgba(255,92,58,0.3)',background:'rgba(255,92,58,0.08)',color:'#ff5c3a',fontFamily:'Syne,sans-serif',fontWeight:600,fontSize:'0.8rem',cursor:'pointer'}}>✓ Confirm Cancel</button>
                </>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:16}}>
              {[
                ...
                {label:'Revenue',value:`$${totalRevenue}`,color:'#ff5c3a',icon:'💰',sub:'FJD total'},
              ].map(s=>(
            ...
            <div style={{marginBottom:16}}>
              ...
              {(()=>{
                ...
                const todayBookings=upcoming.filter(b=>{
                  try{return new Date(b.departure_datetime).toDateString()===today}catch{return false}
                })
                if(todayBookings.length===0) return <div style={{padding:'16px',background:card,borderRadius:10,fontSize:'0.82rem',color:sub,textAlign:'center'}}>No departures today</div>
                return todayBookings.map(b=><BookingCard key={b.id} b={b}/>)
              })()}
            </div>
            ...
            <div style={{background:card,border:`1px solid ${border}`,borderRadius:12,overflow:'hidden'}}>
              {routes.filter(r=>r.active).length===0?(
                <div style={{padding:'20px',fontSize:'0.82rem',color:sub,textAlign:'center'}}>No active routes. Add routes in the Routes tab.</div>
        ...
        {tab==='bookings'&&(
          <div>
            ...
            <div style={{marginBottom:10}}>
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by name, REF, phone..." style={inp}/>
            </div>
            ...
            <input value={filterDate} onChange={e=>setFilterDate(e.target.value)} type="date" placeholder="Filter by date" style={{...inp,marginBottom:12}}/>

            {/* Results summary */}
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
              <span style={{fontSize:'0.78rem',color:sub}}>{filteredBookings.length} bookings found</span>
            ...
            {loading?<div style={{color:sub,textAlign:'center',padding:40}}>Loading...</div>
              :filteredBookings.length===0?<div style={{textAlign:'center',padding:'40px 0',color:sub}}><div style={{fontSize:'2rem',marginBottom:8}}>🔍</div><p>No bookings match your filters</p></div>
            ...
            {pending.length===0?(
              ...
              <div key={b.id} style={{background:card,border:'1px solid rgba(245,200,66,0.3)',borderRadius:14,padding:16,marginBottom:12}}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px 16px',marginBottom:14}}>
                  {[{l:'Passenger',v:b.passenger_name},{l:'REF',v:b.ref},{l:'Phone',v:b.phone},{l:'Requested',v:b.cancellation_requested_at?new Date(b.cancellation_requested_at).toLocaleDateString():'—'}].map(i=>(
                    <div key={i.l}>
                ...
                <div style={{display:'flex',gap:8}}>
                  <button onClick={()=>rejectCancellation(b.id)} style={{flex:1,padding:'10px',borderRadius:10,border:'1px solid rgba(0,165,80,0.3)',background:'rgba(0,165,80,0.08)',color:'#4dd882',fontFamily:'Syne,sans-serif',fontWeight:600,fontSize:'0.82rem',cursor:'pointer'}}>↩ Reject</button>
                  <button onClick={()=>confirmCancellation(b.id)} style={{flex:1,padding:'10px',borderRadius:10,border:'1px solid rgba(255,92,58,0.3)',background:'rgba(255,92,58,0.08)',color:'#ff5c3a',fontFamily:'Syne,sans-serif',fontWeight:600,fontSize:'0.82rem',cursor:'pointer'}}>✓ Confirm Cancel</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {tab==='routes'&&(
          <div>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
              ...
              <button onClick={()=>{setEditingRoute(null);resetRouteForm();setShowRouteForm(true)}} style={{padding:'8px 16px',borderRadius:10,border:'none',background:'#ff5c3a',color:'white',fontFamily:'Syne,sans-serif',fontWeight:700,fontSize:'0.82rem',cursor:'pointer',boxShadow:'0 4px 14px rgba(255,92,58,0.35)'}}>+ Add Route</button>
            </div>

            {/* Route form */}
            {showRouteForm&&(
              <div style={{background:card,border:`1px solid ${border}`,borderRadius:14,padding:16,marginBottom:16}}>
                <div style={{fontFamily:'Syne,sans-serif',fontWeight:700,color:text,marginBottom:14,fontSize:'0.95rem'}}>{editingRoute?'Edit Route':'New Ferry Route'}</div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10}}>
                  <div>
                    <div style={{fontSize:'0.68rem',textTransform:'uppercase',color:sub,marginBottom:4}}>From</div>
                    <select value={routeForm.origin} onChange={e=>setRouteForm(p=>({...p,origin:e.target.value}))} style={sel}>{PORTS.map(p=><option key={p} value={p}>{p}</option>)}</select>
                  </div>
                  <div>
                    <div style={{fontSize:'0.68rem',textTransform:'uppercase',color:sub,marginBottom:4}}>To</div>
                    <select value={routeForm.destination} onChange={e=>setRouteForm(p=>({...p,destination:e.target.value}))} style={sel}>{PORTS.map(p=><option key={p} value={p}>{p}</option>)}</select>
                  </div>
                </div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10}}>
                  <div>
                    <div style={{fontSize:'0.68rem',textTransform:'uppercase',color:sub,marginBottom:4}}>Ferry Name</div>
                    <input value={routeForm.ferry_name} onChange={e=>setRouteForm(p=>({...p,ferry_name:e.target.value}))} placeholder="MV Cagimaira II" style={inp}/>
                  </div>
                  <div>
                    <div style={{fontSize:'0.68rem',textTransform:'uppercase',color:sub,marginBottom:4}}>Operator</div>
                    <input value={routeForm.operator_name} onChange={e=>setRouteForm(p=>({...p,operator_name:e.target.value}))} placeholder="Goundar Shipping" style={inp}/>
                  </div>
                </div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10,marginBottom:10}}>
                  <div>
                    <div style={{fontSize:'0.68rem',textTransform:'uppercase',color:sub,marginBottom:4}}>Departs</div>
                    <input type="time" value={routeForm.departs} onChange={e=>setRouteForm(p=>({...p,departs:e.target.value}))} style={inp}/>
                  </div>
                  <div>
                    <div style={{fontSize:'0.68rem',textTransform:'uppercase',color:sub,marginBottom:4}}>Arrives</div>
                    <input type="time" value={routeForm.arrives} onChange={e=>setRouteForm(p=>({...p,arrives:e.target.value}))} style={inp}/>
                  </div>
                  <div>
                    <div style={{fontSize:'0.68rem',textTransform:'uppercase',color:sub,marginBottom:4}}>Duration</div>
                    <input value={routeForm.duration} onChange={e=>setRouteForm(p=>({...p,duration:e.target.value}))} placeholder="~14 hrs" style={inp}/>
                  </div>
                </div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr',gap:10,marginBottom:10}}>
                  {[{l:'Economy $',k:'economy_fare'},{l:'Cabin $',k:'cabin_fare'},{l:'Economy Seats',k:'seats_economy'},{l:'Cabin Seats',k:'seats_cabin'}].map(f=>(
                    <div key={f.k}>
                      <div style={{fontSize:'0.65rem',textTransform:'uppercase',color:sub,marginBottom:4}}>{f.l}</div>
                      <input type="number" value={(routeForm as any)[f.k]} onChange={e=>setRouteForm(p=>({...p,[f.k]:Number(e.target.value)}))} style={inp}/>
                    </div>
                  ))}
                </div>
            ...
            {routes.length===0?(
              <div style={{textAlign:'center',padding:'40px 0',color:sub}}>
                ...
                <p style={{color:text,fontWeight:600}}>No routes yet</p>
                <p style={{fontSize:'0.82rem',marginTop:6}}>Add your first ferry route above</p>
              </div>
            ):routes.map(r=>(
              <div key={r.id} style={{background:card,border:`1px solid ${r.active?border:'rgba(255,255,255,0.04)'}`,borderRadius:12,padding:'14px 16px',marginBottom:8,opacity:r.active?1:0.6}}>
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
                  ...
                  <div style={{fontSize:'0.75rem',color:sub}}>{r.seats_economy} eco · {r.seats_cabin} cabin seats</div>
                </div>
                <div style={{display:'flex',gap:6}}>
